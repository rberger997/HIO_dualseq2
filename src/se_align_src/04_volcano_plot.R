...
#' ## Load the data
#' Start by making a single volcano plot using `ggplot`. We'll use the SE 8h over SE 2h file to make this.

# Load data 
se <- read.csv(here('results/DESeq2_se/diff_expression/SE_8h_over_SE_2h_diffexpress.csv'), header = T)
...
se <- prep_df(se, label = 'SE', time = '8h')

#' ## Build volcano plot


ggplot(data = se, aes(x=log2FoldChange, y=-log10(padj), 
...
files <- list.files(here('results/DESeq2_se/diff_expression/'))
files
...
for(i in seq(files)){
  ...
  # Load file
  temp <- read.csv(here(paste0('results/DESeq2_se/diff_expression/',file)), header = T)
...
data1$label <- factor(data1$label, levels = c('SE'))
...
#' ## Save png of plot
#+ save, eval=F
png(filename = here("/img/se_align_volcano_facet.png"),
    width = 900, height = 600)
print(p)
dev.off()

# Save data
write.csv(data1, file = here('results/DESeq2_se/volcano_data.csv'))
