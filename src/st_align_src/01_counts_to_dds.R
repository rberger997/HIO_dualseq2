...
dir <- here('results/Run_2374/ST/')


# Where should the output files go?
output_folderID <- 'DESeq2_st'  # Name of output folder
dir.create(here('results', output_folderID))

results.dir <- here('results/DESeq2_st/')


#+ set_cache_dir, include=F
# Set a directory for all cache files to go
dir.create(here('results/DESeq2_st/src_html_output'))
dir.create(here('results/DESeq2_st/src_html_output/knitr_cache'))
...
st.anno <- read.csv(here('data/st_annotation.csv'), stringsAsFactors = FALSE)
...
sample_key <- dplyr::filter(sample_key, dualseq_expt == 2) %>% 
  dplyr::filter(code_name %in% c('ST_2h', 'ST_8h'))
...
# Save output file:
write.csv(ddscounts, file = file.path(results.dir, "ST_align_complete-dataset_DESeq2-normalized-counts.csv"))
...
#' 
#' Alignment to the Typhi genome also generates more false positive reads mapped from human HIO controls (~280 for Typhi, ~5 for STM and SE). These reads were found to be aligned to a single ST gene (WP_000068796.1, cell division protein ftsN).
