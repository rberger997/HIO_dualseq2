...
opts_chunk$set(cache.path = here('results/DESeq2_stm/src_html_output/knitr_cache/'))
...
# Load data 
stm2 <- read.csv(here('results/DESeq2_stm/diff_expression/STM_8h_over_STM_2h_diffexpress.csv'), header = T)
...
stm2 <- prep_df(stm2, label = 'STM', time = '8h')
...
files <- list.files(here('results/DESeq2_stm/diff_expression/'))
files
...
for(i in seq(files)){
  ...
  # Load file
  temp <- read.csv(here(paste0('results/DESeq2_stm/diff_expression/',file)), header = T)
...
data1$label <- factor(data1$label, levels = c('STM','SPI1','SPI2'))
...
#' ## Save png of plot
#+ save, eval=F
png(filename = here("/img/stm_align_volcano_facet.png"),
    width = 900, height = 600)
print(p)
dev.off()

# Save data
write.csv(data1, file = here('results/DESeq2_stm/volcano_data.csv'))
