...
#' 
#' This script analyzes the differential expression files over STM, not PBS control. The original version that addresses PBS is labeled gsea_reactome.R.
#' 
...
dir.create(here('results/DESeq2_human/GSEA/reactome_stm'))
...
for(i in seq(files)){
  ...
  input <- read.csv(file = here('results/DESeq2_human/diff_expression_stm',
                                files[i])) %>%
  ...
  # Save results
  out.dir <- here('results/DESeq2_human/GSEA/reactome_stm/')
  write.csv(temp, 
...
# Save the full data
write.csv(full.data, here('results/DESeq2_human/GSEA/reactome_stm/GSEA_reactome_all.csv'), row.names = F)
...
data2 <- filter(full.data, Description %in% x$Description & label %in% c('SE', 'ST'))
